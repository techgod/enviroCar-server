...
public class TrackProtoTest extends ResourceTestBase {
    ...
    public void testContentNegotiation() throws IOException {
       ...
       ClientResponse response = resource().path("/tracks-proto")
        ...
        response = client().resource(location).accept(MediaTypes.APPLICATION_PROTOBUF).get(ClientResponse.class);
        assertNotNull(response);
        GreetingProtos.Greeting gb = GreetingProtos.Greeting.parseFrom(response.getEntityInputStream());
        assertNotNull(gb.getGreeting());
        assertThat(gb.getGreeting(),equalTo("Hello"));
        //assertThat(response, hasStatus(Response.Status.OK));
        //assertThat(response.getType(), isCompatible(MediaTypes.CSV_TYPE));
